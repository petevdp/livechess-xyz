...
export interface ToggleProps extends ToggleButtonPrimitive.ToggleButtonRootProps, VariantProps<typeof toggleVariants> {}

const Toggle: Component<ToggleProps> = (props) => {
	const [, rest] = splitProps(props, ['class', 'variant', 'size'])
	return <ToggleButtonPrimitive.Root class={cn(toggleVariants({ variant: props.variant, size: props.size }), props.class)} {...rest} />
}
