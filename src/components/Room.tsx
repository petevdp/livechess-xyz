...
function Room() {
	G.setupGame()
	const [roomStatus] = yMapToSignal<R.RoomStatus>(R.room()!.details, 'status', 'pregame')
	return <Switch>
...
function Lobby() {
	...
	return <div class="grid grid-cols-[60%_auto] gap-2">
		<div class="col-span-full grid place-items-center">
			<Button kind="primary" onclick={copyInviteLink}>Copy Invite Link</Button>
		</div>
		<GameConfigForm/>
...
function GameConfigForm() {
	const [variant] = yMapToSignal<G.Variant>(R.room()!.gameConfig as Y.Map<G.Variant>, "variant", "regular")
	const [timeControl,] = yMapToSignal<G.TimeControl>(R.room()!.gameConfig as Y.Map<G.TimeControl>, "timeControl", "5m")
	const [increment] = yMapToSignal<G.Increment>(R.room()!.gameConfig as Y.Map<G.Increment>, "increment", "0")
	...
	return <div>
		<div
			class="grid grid-cols-2 grid-rows-[min-content_auto_min-content] gap-3 ">
			<MultiChoiceButton
				label="Variant"
				classList={{
					"grid": true,
			...
			<MultiChoiceButton
				classList={{"grid": true, "grid-rows-1": true, "grid-cols-5": true, "w-full": true, "text-sm": true}}
				label="Time Control"
				choices={G.TIME_CONTROLS.map(tc => ({label: tc, id: tc}) satisfies Choice<G.TimeControl>)}
				selected={timeControl()} onChange={setConfigValue('timeControl')}/>
			<MultiChoiceButton
				label="Increment"
				classList={{
					"grid": true,
				...
				choices={G.INCREMENTS.map(i => ({label: `${i}s`, id: i}) satisfies Choice<G.Increment>)}
				selected={increment()}
...
function ChatBox() {
	...
	return <div class="h-full flex flex-col">
		<div ref={chatFeed} class="flex flex-col grow overflow-y-scroll h-48 mb-2">
			<For each={messages}>{(message) => <ChatMessage message={message}/>}</For>
		</div>
		<form onSubmit={sendMessage} class="flex w-full h-9">
			<input class="grow bg-inherit border-solid border-[1px] border-gray-300 rounded p-1 w-max mr-1" type="text"
...
function ChatMessage(props: { message: R.ChatMessage }) {
	return <div>
		<Show when={props.message.sender}><b>{props.message.sender}:</b> </Show>
		<Switch>
			<Match when={props.message.sender}>{props.message.text}</Match>
			<Match when={!props.message.sender}><i>{props.message.text}</i></Match>
		</Switch>
	</div>
}
...
function NickForm() {
	...
	return <form onSubmit={onSubmit}>
		...
		<input type="text"
					 class="bg-gray-800 text-white p-1"
					 value={displayName()}
					 required={true}
					 pattern={"[a-zA-Z0-9]+"}
