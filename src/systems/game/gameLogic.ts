...
export function candidateMoveToMove(candidateMove: CandidateMove, promotion?: PromotionPiece): Move {
	return {
...
export const GAME_ENDED_REASONS = ['checkmate', 'stalemate', 'threefold-repetition', 'insufficient-material', 'draw-agreed', 'timeout', 'resigned'] as const
...
export function validateAndPlayMove(from: string, to: string, game: Game, promotionPiece?: PromotionPiece) {
...
export function applyMoveToBoard(move: CandidateMove | Move, board: Board) {
	...
	if (piece.type === 'pawn' && (_move.to.y === 0 || _move.to.y === 7)) {
		newBoard.pieces[moveToCoords] = {color: board.toMove, type: _move.promotion || 'queen'}
		promoted = true
	}
...
export function getLegalMoves(piecePositions: Coords[], game: Game): CandidateMove[] {
	...
	for (const start of piecePositions) {
		...
		candidateMoves = [...candidateMoves, ...(getMovesFromCoords(start, game.board, game.moveHistory, piece))]
	}
...
function noMoves(game: Game) {
	let legalMoves = getLegalMoves(Object.keys(game.board.pieces).map(n => coordsFromNotation(n)), game);
