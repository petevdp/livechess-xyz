...
import {connectToRoom, createId, createRoom, getPage} from "../../utils/testUtils.ts";
...
async function startGame(player1: string, player2: string, player1Page: Page, player2Page: Page) {
...
async function makeMove(page: Page, from: string, to: string, promotion?: PromotionPiece) {
...
describe("game", async () => {
	...
	test("make a move", async () => {
		...
		const expectedBoard = expectedGame.boardHistory[expectedGame.boardHistory.length - 1][1];
		...
		expect(game.boardHistory[game.boardHistory.length - 1][1]).toEqual(expectedBoard);
